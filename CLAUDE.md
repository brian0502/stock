...
| 2 | 五檔「剛突破＋基本面強」建議標的 | market-opportunity-scan v2.0 | 逐檔附：即時價／技術型態／基本面／進場參考價／停損；讓用戶自行決定，Claude 不代下單；沒有符合三條件的就直說，不硬湊。**🆕 必先逐檔複查既有 watchlist（重抓即時價→keep/remove，崩跌/跌破買價/失突破動能者剔除、不沿用舊價，台股美股都做）；台股候選用「投信買超＋主動式 ETF 近期增持」當漏斗。詳 current_strategy.md ②。** **🚩 2026-06-15 兩條鐵律（用戶第 N 次糾正、跨電腦適用）：(a) 就算結論「不進場」，台股權值股（台積電 2330／鴻海 2317／聯發科 2454 等指數驅動股）也一定要逐檔列進推薦清單、給即時價＋分層建議買價＋MA5/10/20/60 停損——「不進場」是判斷結果、不是略過分析的藉口，禁用「台股候選掛零」一句帶過。(b) 五檔候選必須「真的去跑投信買超排行＋主動式 ETF(00981A/00980A 等)近期增持」這個漏斗產生，禁止「只拿用戶點名的＋沿用舊 watchlist 湊數」；若當下抓不到漏斗數據要明講「漏斗未跑/數據未取得」、不可假裝跑過。** |
