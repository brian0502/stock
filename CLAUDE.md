...
- 🌐 **系統事件台美同表同處置（2026-09-10 用戶定案）**：Layer 2 不分帳戶＝查證表同列台／美數據、觸發＝兩帳戶同時全清評估；油價閘門＝雙市場閘門（Brent ≥$100 或 WTI 單日 +5%＝台股新單＋美股新單同時暫停）。
...
## 🧮 Token 紀律（2026-09-10 用戶定案・每次分析強制）

> 起因：9/9 單次分析 77 turns、快取讀 12M tokens、輸出 22 萬＝用戶動怒。錢不是花在查資料，是花在「每個小步驟都回頭重讀整個 context」＋「每列堆歷史敘事」＋「先寫佔位再補」。

1. **一次分析 ≤20 turns**：資料抓取一律用 repo 內固定腳本（見下「固定工具腳本」），不在 session 內重寫腳本；TWSE＋分點＋Yahoo 三支腳本各跑一次。
2. **研究只派 1 個子代理**（油價/Fed/台股新聞/股癌合併一題），不派 4 個。
3. **讀檔只 grep 需要的列**（🧊 表特定列、actionBoard 特定 row、持股現況特定列），禁止整段 sed 8K+ 字。
4. **報告＋data.js＋current_strategy＋dashboard＋episodes-read 用一支 node 腳本一次寫完**，不留 {{佔位}} 再 Edit、不逐檔多次 Edit。
5. **列只留數字**：actionBoard／current_strategy 每列＝動作、價、股數、停損、一行條件＋關鍵法人數字（≤300 字）；歷史敘事不層層前綴（「前次」最多留 1 段，其餘靠 archive/ 與 git log）。
6. **回覆只給**行動總表＋股癌＋五檔＋組合/Layer 2 四塊，不重述分析細節。
7. 中斷後別閒置超過 1 小時（快取過期＝整個 context 重寫 50 萬）。

### 固定工具腳本（tools/・2026-09-10 入庫，不要再重寫）
